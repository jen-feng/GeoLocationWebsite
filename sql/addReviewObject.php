...
function updateStoreRating($pdo1) {
	...
	$stmnt1 = $pdo1->prepare($s1);
	$stmnt1->execute([$_SESSION['store_id']]);
	$store_rating = $stmnt1->fetchColumn();
	$stmnt1 = $pdo1->prepare($s2);
	$stmnt1->execute([$store_rating, $_SESSION['store_id']]);
	return $store_rating;
}
?>